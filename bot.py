...
# БАЗА ПРЕДМЕТОВ: ТОЛЬКО ДЛЯ 6 ЛЕТ (МАКСИМАЛЬНО ПРОСТО И ЖИЗНЕННО)
TOPICS_POOL = {
    "Математика": [
        "Простое сложение яблок или конфет (до 10)",
        "Простое вычитание игрушек (до 10)",
        "Базовые фигуры (найди круг, квадрат, треугольник)",
        "Счет предметов (что больше, что меньше)"
    ],
    "Животный мир": [
        "Кто что ест (зайцы, медведи, белочки)",
        "Где кто живет (берлога, дупло, гнездо)",
        "Домашние животные и их звуки (кошки, собаки, коровы)",
        "Кто спит зимой, а кто нет"
    ],
    "География и Земля": [
        "Времена года (зима, весна, лето, осень)",
        "Природные явления (дождь, снег, радуга)",
        "Где холодно, а где жарко (Африка, Северный полюс)",
        "День и ночь (Солнце и Луна)"
    ],
    "Окружающий мир": [
        "Правила светофора (красный, желтый, зеленый)",
        "Профессии (врач лечит, повар готовит, строитель строит)",
        "Овощи и фрукты (что сладкое, что соленое)",
        "Транспорт (что летает, что ездит по земле)"
    ]
}
...
async def get_question(request: Request):
    try:
        ...
        system_prompt = f"""
        Ты — профессор Фил, добрый учитель для девочки 6 лет (зовут {user_name}).

        ЗАДАЧА:
        Сгенерируй ОЧЕНЬ ПРОСТОЙ детский вопрос по теме.
        ...
        1. ЯЗЫК: Пиши ИСКЛЮЧИТЕЛЬНО на грамотном русском языке. КАТЕГОРИЧЕСКИ ЗАПРЕЩЕНО использовать английские слова, китайские иероглифы или выдуманные слова.
        2. ВОЗРАСТ 6 ЛЕТ: Вопросы должны быть житейскими и понятными. Кто говорит "Мяу"? Какого цвета снег? Что делает врач? Никаких сложных фактов!
        3. МАТЕМАТИКА: Если это математика, используй ТОЛЬКО ОДНО действие (например: "Было 3 яблока, 1 съели. Сколько осталось?"). Считать только до 10!
        4. ПРОВЕРКА ОТВЕТА: Убедись на 100%, что правильный ответ физически присутствует в массиве "options"!
        ...
        completion = groq_client.chat.completions.create(
            ...
            temperature=0.2 # КРИТИЧЕСКИ ВАЖНО: Низкая температура убирает иероглифы и ошибки в математике!
        )
        ...
        if len(recent_subtopics_list) > 20:
            recent_subtopics_list = recent_subtopics_list[-20:]
...
async def chat_with_robot(request: Request):
    try:
        ...
        system_prompt = f"""
        Ты — Профессор Фил, добрый сова-наставник для девочки {user_name} (6 лет).
